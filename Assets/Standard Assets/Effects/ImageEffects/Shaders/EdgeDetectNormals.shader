...
Shader "Hidden/EdgeDetect" { 
	...
	{	
		...
		//float depth = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, i.uv[1]);
		//depth = 1 - Linear01Depth(depth);
		//return float4(depth, depth, depth, 1);

		float f = (depthsDiag > centerDepth.xxxx) ? depthsDiag : centerDepth.xxxx;
		//depthsDiag = lerp(depthsDiag, f, 0.5);

...
Shader "UnityChan/UCTS_Standard_EyeWhite" {
    Properties {
        ...
        _HighColor_s1 ("1st_HighShade", Color) = (1,1,1,1)
        _HighColor_Power_s1 ("1st_HighShade_Power", Range(0.08, 0.12)) = 0.1
        _HighFeather_s1 ("1st_HighFeather", Range(0, 0.1)) = 0
...
////// Lighting:
                ...
                float3 finalColor = saturate((1.0-(1.0-saturate(_RimLight_var))*(1.0-(DecodeLightProbe( normalDirection )*_GI_Intensity))));
        ...
        uniform float4 _HighColor_s1;
        uniform float _HighColor_Power_s1;
        uniform float _HighFeather_s1;
				...
				float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                    ...
                    float sample = mask * lerp(spec, high, _Is_SpecularToHighColor);
					float3 highColor = lerp(_HighColor.rgb, (_HighColor.rgb * lightColor), _Is_LightColor_HighColor);
					//highColor += lerp(saturate(0 - sample), 0, _Is_BlendAddToHiColor); 混合



                    highStep = _HighColor_Power_s1;
                    highStep = pow(highStep, exp2(lerp(10,1, highStep * 10)));
                    spec = saturate(1 - ((1 - proj) - highStep) / _HighFeather_s1);

                    high = pow(proj, exp2(lerp(11, 1, _HighColor_Power_s1)));
                    float sample2 = mask * lerp(spec, high, _Is_SpecularToHighColor);
					float3 highColor2 = lerp(_HighColor_s1.rgb, (_HighColor_s1.rgb * lightColor), _Is_LightColor_HighColor);
					//highColor += lerp(saturate(0 - sample), 0, _Is_BlendAddToHiColor); 混合


                    highColor = lerp(highColor, highColor2, sample2 * _HighColor_s1.a);

					fixed4 finalRGBA = float4(highColor.rgb, sample * _HighColor.a);

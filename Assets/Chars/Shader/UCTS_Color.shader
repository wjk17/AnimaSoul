...
//ToonCoreStart
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                //normalDirection = i.normalDir;//normalLocal;
...
////// Lighting:
                ...
                float3 n = lerp( i.normalDir, normalDirection, _Is_NormalMapToHighColor );
                float spec = 0.5*dot(halfVL, n)+0.5; //  Specular
                //float spec = 0.5*dot(halfVL, i.normalDir)+0.5; //  Specular
                ...
                //float rimProj = (1.0 - dot(normalDirection, viewDirection));
                ...
                fixed4 finalRGBAMin = 0.01;//fixed4(0.2, 0.2, 0.2, 0.2);
                
                fixed4 finalRGBA = fixed4(rim,1);

                //UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                
                finalRGBA.r = max(finalRGBA.r, finalRGBAMin.r);
                finalRGBA.g = max(finalRGBA.g, finalRGBAMin.g);
                finalRGBA.b = max(finalRGBA.b, finalRGBAMin.b);
                finalRGBA.a = max(finalRGBA.a, finalRGBAMin.a);

...
Shader "UnityChan/UCTS_Standard_MatCapMulty" {
    Properties {
...
//�����܂�.
        ...
        _MatCap_Sampler_Mul ("MatCap_Sampler_Mul", 2D) = "white" {}
        _MatCapColor_Mul ("MatCapColor_Mul", Color) = (1,1,1,1)
		...
		[MaterialToggle] _Is_BlendOverlayToMatCap("Is_BlendOverlayToMatCap", Float) = 1
        _MatCapThreshold("MatCapThreshold", Float) = 0.05
		_ShadowOut_Alpha("ShadowOut_Alpha", Float) = 0.5
...
//ToonCoreStart
        Pass {
            ...
            uniform sampler2D _MatCap_Sampler_Mul; uniform float4 _MatCap_Sampler_Mul_ST;
            uniform float4 _MatCapColor_Mul;
			...
			uniform fixed _Is_BlendOverlayToMatCap;
            ...
            uniform float _MatCapThreshold;
   			uniform float _ShadowOut_Alpha;
...
////// Lighting:
                ...
                float shadow = saturate((node_8323 + ( (lerp( node_4315, (node_4315*saturate(((attenuation*node_3494)+node_3494+_Tweak_SystemShadowsLevel))), _Set_SystemShadowsToBase ) - node_583) * ((1.0 - _Set_1st_ShadePosition_var.rgb).r - node_8323) ) / (_BaseColor_Step - node_583)));
                float3 node_1702 = lerp(Set_BaseColor,lerp(Set_1st_ShadeColor,Set_2nd_ShadeColor,saturate((node_9309 + ( (node_4315 - node_2294) * ((1.0 - _Set_2nd_ShadePosition_var.rgb).r - node_9309) ) / (_ShadeColor_Step - node_2294)))),shadow); // Final Color
                ...
                float3 Set_HighColor = (lerp( saturate((node_1702-node_5489)), node_1702, _Is_BlendAddToHiColor )+lerp( node_5205, (node_5205*((1.0 - shadow)+(shadow*_TweakHighColorOnShadow))), _Is_UseTweakHighColorOnShadow ));
                ...
                float node_254_ang = (_Rotate_MatCapUV * pi);
                float node_254_spd = 1.0;
                float node_254_cos = cos(node_254_spd*node_254_ang);
                float node_254_sin = sin(node_254_spd*node_254_ang);
                
                float2 piv_0p5 = float2(0.5,0.5);

                float node_5552_ang = (_Rotate_NormalMapForMatCapUV * pi);
                ...
                float2 node_5552 = (mul(Set_UV0-piv_0p5,float2x2( node_5552_cos, -node_5552_sin, node_5552_sin, node_5552_cos))+piv_0p5);
                float3 _NormalMapForMatCap_var = UnpackNormal(tex2D(_NormalMapForMatCap,TRANSFORM_TEX(node_5552, _NormalMapForMatCap)));

                float tweak = (0+_Tweak_MatCapUV);

                float2 matcap_uv = (mul((0 + ( ((mul( UNITY_MATRIX_V, float4(lerp( i.normalDir, mul( _NormalMapForMatCap_var.rgb, tangentTransform ).xyz.rgb, _Is_NormalMapForMatCap ),0) ).xyz.rgb.rg*0.5+0.5) - tweak) * (1.0 - 0) ) / ((1.0-_Tweak_MatCapUV) - tweak))-piv_0p5,float2x2( node_254_cos, -node_254_sin, node_254_sin, node_254_cos))+piv_0p5);
                
                float4 matCapMap = tex2D(_MatCap_Sampler,TRANSFORM_TEX(matcap_uv, _MatCap_Sampler));
                float3 matCapClr = (matCapMap.rgb*_MatCapColor.rgb);
                float3 matCapLightClr = lerp( matCapClr, (matCapClr*Set_LightColor), _Is_LightColor_MatCap );

                float4 matCapMap_Mul = tex2D(_MatCap_Sampler_Mul,TRANSFORM_TEX(matcap_uv, _MatCap_Sampler_Mul));
                float3 matCapClr_Mul = (matCapMap_Mul.rgb*_MatCapColor_Mul.rgb);
                float3 matCapLightClr_Mul = lerp( matCapClr_Mul, (matCapClr_Mul*Set_LightColor), _Is_LightColor_MatCap );

                float3 matCap = lerp( matCapLightClr, matCapLightClr*((1.0 - shadow)+(shadow*_TweakMatCapOnShadow)), _Is_UseTweakMatCapOnShadow );
                float3 matCapMul = matCapLightClr_Mul;//lerp( matCapLightClr_Mul, lerp(matCapLightClr_Mul, 1.0, shadow * _TweakMatCapOnShadow), _Is_UseTweakMatCapOnShadow );

                // float MatCapAplha = matCapMap.a;
				 //float3 matCapResult = lerp((_RimLight_var*matCap), (_RimLight_var + matCap), _Is_BlendAddToMatCap);
				 float3 base = _RimLight_var;
				...
				// float3 Screen = 1 - (1 - base)*(1 - blend);
				// float3 Darken = min(base, blend);
				// float3 Lighten = max(base, blend);

				// float3 MixResult2 = Overlay;
				// float3 MixResult = lerp(Multiply, Add, _Is_BlendAddToMatCap);
				// MixResult = lerp(MixResult, MixResult2, _Is_BlendOverlayToMatCap);

				// fixed stepShadowOut = step(_MatCapThreshold, base);				
				// float3 shadowOut = lerp(lerp(base, MixResult, _ShadowOut_Alpha), MixResult, stepShadowOut);

				// float3 matCapResult = lerp(base, shadowOut, MatCapAplha);
                // float3 matCapResult = Multiply; 
                float3 matCapResult;
                matCapResult = base;
                matCapResult += matCap; 
                //matCapResult *= matCapMul; 
                float3 finalColor = saturate((1.0-(1.0-saturate(lerp( _RimLight_var, matCapResult, _MatCap )))*(1.0-(DecodeLightProbe( normalDirection )*_GI_Intensity))));                

                float MatCapAplha = matCapMap.a;

                // float3 finalColor = saturate((1.0-(1.0-saturate(lerp( _RimLight_var, lerp( (_RimLight_var*matCap), (_RimLight_var+matCap), _Is_BlendAddToMatCap ), _MatCap )))*(1.0-(DecodeLightProbe( normalDirection )*_GI_Intensity))));
                finalColor = lerp(_RimLight_var, finalColor, MatCapAplha);
                // finalColor = _RimLight_var;
                fixed4 finalRGBA = fixed4(lerp(finalColor, finalColor * matCapMul, matCapMap_Mul.a),1);

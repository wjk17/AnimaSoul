...
Shader "UnityChan/UCTS_Standard_Z" {
    Properties {
        ...
        _HighColor2 ("HighColor2", Color) = (1,1,1,1)
        [MaterialToggle] _Is_LightColor_HighColor2 ("Is_LightColor_HighColor2", Float ) = 1
        [MaterialToggle] _Is_NormalMapToHighColor2 ("Is_NormalMapToHighColor2", Float ) = 0
        _HighColor_Power2 ("HighColor_Power2", Range(0, 1)) = 0
        [MaterialToggle] _Is_SpecularToHighColor2 ("Is_SpecularToHighColor2", Float ) = 0
        [MaterialToggle] _Is_BlendAddToHiColor2 ("Is_BlendAddToHiColor2", Float ) = 0
        [MaterialToggle] _Is_UseTweakHighColorOnShadow2 ("Is_UseTweakHighColorOnShadow2", Float ) = 0
        _TweakHighColorOnShadow2 ("TweakHighColorOnShadow2", Range(0, 1)) = 0
...
//ToonCoreStart
        Pass {
            ...
            uniform float _HighColor_Power2;
            uniform fixed _Is_BlendAddToHiColor2;
            uniform fixed _Is_UseTweakHighColorOnShadow2;
            uniform fixed _Is_SpecularToHighColor2;
            uniform fixed _Is_NormalMapToHighColor2;
            uniform float _TweakHighColorOnShadow2;
            uniform fixed _Is_LightColor_HighColor2;
...
////// Lighting:
                ...
                // eye hl
                node_1331 = 0.5*dot(halfDirection,lerp( i.normalDir, normalDirection, _Is_NormalMapToHighColor2 ))+0.5; //  Specular
                node_5489 = (saturate((_Set_HighColorMask_var.g+_Tweak_HighColorMaskLevel))*lerp( (1.0 - step(node_1331,(1.0 - _HighColor_Power2))), pow(node_1331,exp2(lerp(11,1,_HighColor_Power2))), _Is_SpecularToHighColor2 ));
                node_5205 = (lerp( _HighColor2.rgb, (_HighColor2.rgb*Set_LightColor), _Is_LightColor_HighColor2 )*node_5489);
                node_2595 = Set_FinalShadowSample;
                Set_HighColor = (lerp( saturate((node_1702-node_5489)), node_1702, _Is_BlendAddToHiColor )+lerp( node_5205, (node_5205*((1.0 - node_2595)+(node_2595*_TweakHighColorOnShadow2))), _Is_UseTweakHighColorOnShadow ));
                float3 node_23792 = Set_HighColor;
                ...
                node_2379 = node_2379 + node_23792;
                ...
                // float MatCapAplha = _MatCap_Sampler_var.a;
				// //float3 RimMixMatCap = lerp((_RimLight_var*node_4172), (_RimLight_var + node_4172), _Is_BlendAddToMatCap);
				// float3 base = _RimLight_var;
				// float3 blend = node_4172;
				// float3 Multiply = base * blend;
				// float3 Add = base + blend;
				...
				// float3 RimMixMatCap = lerp(base, shadowOut, MatCapAplha);
                // float3 finalColor = saturate((1.0-(1.0-saturate(lerp( _RimLight_var, RimMixMatCap, _MatCap )))*(1.0-(DecodeLightProbe( normalDirection )*_GI_Intensity))));

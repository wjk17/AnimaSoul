...
Shader "UnityChan/UCTS_Standard_StencilMask" {
    Properties {
...
////// Lighting:
			...
			Pass {
				...
				float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
					...
					finalRGBA = float4(highColor, sample * _HighColor.a);//highColor.r);
					//finalRGBA = float4(highColor, 0);
					if (_HighColor_Power < 0.001) discard;
					//if (finalRGBA.g < 0.1) discard;// return float4(1, 1, 1, 0);

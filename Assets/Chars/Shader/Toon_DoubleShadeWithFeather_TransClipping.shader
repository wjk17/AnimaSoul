...
////// Lighting:
                ...
                float node_4032 = (saturate((_Set_HighColorMask_var.g+_Tweak_HighColorMaskLevel))*lerp( (1.0 - step(node_1331,(1.0 - _HighColor_Power))), pow(node_1331,exp2(lerp(11,1,_HighColor_Power))), _Is_SpecularToHighColor ));
                float3 node_5205 = (lerp( _HighColor.rgb, (_HighColor.rgb*Set_LightColor), _Is_LightColor_HighColor )*node_4032);
                float node_2595 = Set_FinalShadowSample;
                float3 Set_HighColor = (lerp( saturate((node_1702-node_4032)), node_1702, _Is_BlendAddToHiColor )+lerp( node_5205, (node_5205*((1.0 - node_2595)+(node_2595*_TweakHighColorOnShadow))), _Is_UseTweakHighColorOnShadow ));
                float3 node_3558 = Set_HighColor;
                float2 node_722 = Set_UV0;
                float4 _Set_RimLightMask_var = tex2D(_Set_RimLightMask,TRANSFORM_TEX(node_722, _Set_RimLightMask));
                ...
                float node_8606 = 1.0;
                float node_9273 = 0.0;
                float node_4350 = saturate(lerp( (node_9273 + ( (node_7879 - _RimLight_InsideMask) * (node_8606 - node_9273) ) / (node_8606 - _RimLight_InsideMask)), step(_RimLight_InsideMask,node_7879), _RimLight_FeatherOff ));
                float node_175 = 0.5*dot(i.normalDir,lightDirection)+0.5;
                float3 _LightDirection_MaskOn_var = lerp( (_Is_LightColor_RimLight_var*node_4350), (_Is_LightColor_RimLight_var*saturate((node_4350-((1.0 - node_175)+_Tweak_LightDirection_MaskLevel)))), _LightDirection_MaskOn );
                float node_3377 = pow(node_2652,exp2(lerp(3,0,_Ap_RimLight_Power)));
                float3 Set_RimLight = (saturate((_Set_RimLightMask_var.g+_Tweak_RimLightMaskLevel))*lerp( _LightDirection_MaskOn_var, (_LightDirection_MaskOn_var+(lerp( _Ap_RimLightColor.rgb, (_Ap_RimLightColor.rgb*Set_LightColor), _Is_LightColor_Ap_RimLight )*saturate((lerp( (node_9273 + ( (node_3377 - _RimLight_InsideMask) * (node_8606 - node_9273) ) / (node_8606 - _RimLight_InsideMask)), step(_RimLight_InsideMask,node_3377), _Ap_RimLight_FeatherOff )-(saturate(node_175)+_Tweak_LightDirection_MaskLevel))))), _Add_Antipodean_RimLight ));

...
namespace UnityChan
{
    public class SpringBone : MonoBehaviour
    {
        ...
        public void UpdateSpring()
        {
            ...
            if (managerRef.meshCollider)
            {
                var dir = (currTipPos - temp).normalized;
                var mask = 1 << LayerMask.NameToLayer("Paintable");
                var dist = Vector3.Distance(temp, currTipPos) + managerRef.posOs;
                Ray ray = new Ray(temp - dir * dist, dir);
                RaycastHit hit;
                bool bHit = Physics.Raycast(ray, out hit, dist + managerRef.rayOs, mask, QueryTriggerInteraction.Collide);
                if (bHit)
                {
                    currTipPos = hit.point;
                }
                currTipPos = ((currTipPos - trs.position).normalized * springLength) + trs.position;//修正头发长度;            
            }

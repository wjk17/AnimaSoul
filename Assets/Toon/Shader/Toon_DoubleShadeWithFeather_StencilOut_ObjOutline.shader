...
Shader "UnityChanToonShader/Toon_DoubleShadeWithFeather_StencilOut_ObjOutline" {
    Properties {
        _StencilNo ("Stencil No", int) =1
		_StencilNoOutline("Stencil No Outline", int) = 2
...
//ここまで.
		...
		_node_5795("node_5795", Color) = (0.5,0.5,0.5,1)
		_node_9973("node_9973", Float) = 0.05
    }
    SubShader {
        Tags {
        ...
        Pass {
			...
			Pass{
				...
				Stencil{
					...
					#pragma target 3.0
					uniform float4 _node_5795;
				uniform float _node_9973;
				struct VertexInput {
					float4 vertex : POSITION;
					float3 normal : NORMAL;
				};
				...
				VertexOutput vert(VertexInput v) {
					VertexOutput o = (VertexOutput)0;
					o.pos = UnityObjectToClipPos(float4(v.vertex.xyz + v.normal*_node_9973,1));
					UNITY_TRANSFER_FOG(o,o.pos);
					return o;
				}
				float4 frag(VertexOutput i) : COLOR{
					return fixed4(_node_5795.rgb,0);
				}
					ENDCG
				}
//ToonCoreEnd
